...
late StreamSubscription subscription;
  ...
  void initState() {
    ...
    InternetConnectionChecker().hasConnection.then((value) {
      ...
      _initializeData(); // Fungsi baru untuk mengatur alur data
    });
...
void getConnectivity() {
    subscription = InternetConnectionChecker().onStatusChange.listen((status) {
      setState(() {
        ...
        if (isDeviceConnected) {
          _refreshDataOnline(); // Refresh data jika internet kembali
        }
      });
    });
  }
  ...
  Future<void> _initializeData() async {
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      // 1. Load dari Cache dulu (Offline)
      await _loadFromCache();

      // 2. Jika Online, ambil data terbaru
      if (isDeviceConnected) {
        await _refreshDataOnline();
      }
  ...
  Future<void> _loadFromCache() async {
    ...
    // Load Detail Karyawan
    String? cachedDetails = prefs.getString('cache_emp_details_$empId');
    if (cachedDetails != null) {
      setState(() {
        employeeDetails = jsonDecode(cachedDetails);
      });
      // Load Info Pekerjaan
      String? cachedWork = prefs.getString('cache_emp_work_$empId');
      if (cachedWork != null) {
        setState(() => employeeWorkInfoRecord = jsonDecode(cachedWork));
      }
      // Load Info Bank
      String? cachedBank = prefs.getString('cache_emp_bank_$empId');
      if (cachedBank != null) {
        setState(() => employeeBankRecord = jsonDecode(cachedBank));
      }
    }
  }

  Future<void> _refreshDataOnline() async {
    // Panggil semua fungsi fetch API yang ada
...
Future<void> getEmployeeDetails() async {
    ...
    // Cek koneksi, jika offline hentikan proses network
    ...
    // ... (kode URI dan Request HTTP tetap sama) ...
    ...
    if (response.statusCode == 200) {
      // --- TAMBAHKAN BARIS INI (SIMPAN CACHE) ---
      await prefs.setString('cache_emp_details_$empId', response.body);
      // ------------------------------------------
  ...
  Future<void> _pickImage(int id) async {
    // --- TAMBAHKAN CEK KONEKSI INI ---
    if (!isDeviceConnected) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Cannot upload image while offline'),
...
void _showEditOptions(BuildContext context, employeeDetails, firstName,
  ...
  Widget build(BuildContext context) {
   ...
   appBar: AppBar(
        ...
        backgroundColor: Colors.red,
        actions: [
          // --- TAMBAHKAN INDIKATOR INI ---
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 12.0),
            child: Tooltip(
              message: isDeviceConnected ? 'Online' : 'Offline Mode',
              child: Container(
                width: 12,
                height: 12,
                decoration: BoxDecoration(
                  shape: BoxShape.circle,
                  // Hijau = Online, Kuning = Offline
